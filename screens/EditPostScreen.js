...
export default EditPostScreen = function ({navigation, route}) {
  ...
  const [Ingredient, setIngredient] = useState(
    route.params.item.postFoodIngredient,
  );
  ...
  const [defaultRating, setdefaulRating] = useState(
    route.params.item.postFoodRating,
  );
  ...
  const TextChangeMaking = Text => {
    setMaking(Text);
  };
  const TextChangeSummary = Text => {
    setSummary(Text);
  };
  const TextChangetotal = Text => {
    setTotal(Text);
  };
  const TextChangecal = Text => {
    setCal(Text);
  };
  const TextChangeprep = Text => {
    setPrep(Text);
  };
  const TextChangecooking = Text => {
    setCookingtime(Text);
  };
  ...
  const editPost = async () => {
    if (allowPost() == false) {
      Alert.alert(
        'Note',
        'For a relatively complete post, please enter all the information in the marked place',
      );
      return;
    }
    try {
      if (image.length == 0) SavePost();
      else
  ...
  const [mainingredient, setmainingredient] = useState([
    {key: 'Beans & Peas', tick: false},
    {key: ' Beef', tick: false},
    {key: 'Chicken', tick: false},
    {key: 'Egg', tick: false},
    {key: 'Seafood', tick: false},
    {key: 'Pork', tick: false},
    {key: 'Pasta', tick: false},
  ]);
  const [diettype, setdiettype] = useState([
    {key: 'Low-Fat', tick: false},
    {key: 'High-Protein', tick: false},
    {key: 'Vegetarian', tick: false},
    {key: 'Keto', tick: false},
    {key: 'Mediterranean', tick: false},
    {key: 'High-Fiber', tick: false},
  ]);
  const [mealtype, setmealtype] = useState([
    {key: 'Breakfast', tick: false},
    {key: 'Lunch', tick: false},
    {key: 'Dinner', tick: false},
    {key: 'Snack', tick: false},
  ]);
  const [cookingstyle, setcookingstyle] = useState([
    {key: 'Fast Prep', tick: false},
    {key: 'No Cooking', tick: false},
    {key: ' Fast & Easy', tick: false},
    {key: 'Slow Cooker', tick: false},
    {key: 'Grilling', tick: false},
  ]);
  const [course, setcourse] = useState([
    {key: 'Salads & Dressings', tick: false},
    {key: 'Desserts', tick: false},
    {key: 'Sides', tick: false},
    {key: 'Beverages & Smoothies', tick: false},
    {key: 'Soups & Stews', tick: false},
  ]);

  const Hashtags = ({each}) => (
    <TouchableOpacity
      style={{
        backgroundColor: each.tick ? '#9ACD32' : '#E6E6FA',
        marginLeft: 15,
        marginTop: 10,
        alignItems: 'center',
        borderRadius: 15,
    ...
    <View
      ...
      <View
        ...
        <Picker
          ...
          onValueChange={value => {
            (itemI.dv = value), setSelectedValue(value), console.log(itemI);
          }}
    ...
    <View
      ...
      <View
        ...
        <TouchableOpacity onPress={() => navigation.goBack()}>
          <Icon
            name={'arrow-left'}
            style={{color: '#333', fontSize: 25, padding: 5}}
          />
        </TouchableOpacity>
              ...
              <View
                ...
                <ScrollView>
                  <Text
                    ...
                    {language === 'vn' ? 'Tên món ăn: *' : 'Food name: *'}
                  </Text>
                  <TextInput
                    style={{
                      fontSize: 16,
                  ...
                  <Text
                    ...
                    {language === 'vn' ? 'Độ khó *' : 'Difficulty level *'}
                  </Text>
                  <CustomRatingBar />
              ...
              <View
                ...
                <ScrollView>
                  ...
                  <Text
                    ...
                    {language === 'vn'
                      ? 'Công thức làm món ăn này là gì? *'
                      : 'Give me the recipe of your food *'}
                  </Text>
                  <TextInput
                    multiline={true}
                    style={{
            ...
            {loading && (
              <ScrollView>
                <Text
                  style={[
                    styles.TextStyle,
                    {marginTop: 20, color: theme === 'light' ? '#000' : '#fff'},
                  ]}>
                  Meal Type
                </Text>
                <FlatList
        ...
        <View style={styles.popover}>
          <TouchableOpacity onPress={takePhotoFromCamera}>
            <View style={styles.popoverItem}>
              ...
              <Text style={{fontSize: 16, marginTop: 8, color: 'black'}}>
                Take photo
              </Text>
            </View>
          </TouchableOpacity>
